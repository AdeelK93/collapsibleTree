...
collapsibleTree(
  ...
  fill = c(
    "white", # The root
    rep("firebrick", length(unique(df$continent))), # Unique continents
    rep("steelblue", length(unique(paste(df$continent, df$type)))), # Unique types per continent
    rep("green", length(unique(df$country))) # Unique countries
  )
)
```
